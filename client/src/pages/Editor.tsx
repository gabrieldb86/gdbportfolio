      ...
      <main className="editor-main">
        ...
        <section className="editor-panel" aria-labelledby="identity-title">
          ...
          <div className="editor-fields editor-fields-identity">
            <div className="editor-photo-stack">
              <label className="editor-field editor-photo-field"><span>Foto de perfil</span><div className="editor-photo-preview"><img src={config.profilePhoto} alt="Prévia do perfil" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = defaultSiteConfig.profilePhoto; }} /></div><label className="upload-button"><Upload size={14} /> Trocar foto<input type="file" accept="image/*" onChange={(event) => uploadImage(event, (url) => { setConfig((current) => ({ ...current, profilePhoto: url })); setSaved(false); })} /></label><small>Usada no CV e na apresentação pessoal.</small></label>
              <label className="editor-field editor-photo-field"><span>Imagem de destaque</span><div className="editor-photo-preview editor-hero-preview"><img src={config.heroImage} alt="Prévia do projeto de destaque" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = defaultSiteConfig.profilePhoto; }} /></div><label className="upload-button"><ImagePlus size={14} /> Trocar imagem<input type="file" accept="image/*" onChange={(event) => uploadImage(event, (url) => { setConfig((current) => ({ ...current, heroImage: url })); setSaved(false); })} /></label><small>Usada na primeira dobra da home.</small></label>
            </div>
          ...
          <div className="editor-projects">
            {config.projects.map((project, index) => (
              <article className={`editor-project ${project.visible ? "" : "editor-project-hidden"}`} key={`${project.number}-${project.title}`}>
                <div className="editor-project-thumb">{project.image.startsWith("internal:") ? <div className="editor-project-thumb-fallback" aria-label={`Capa interna: ${project.title}`}><strong>{project.title}</strong></div> : <img src={project.image} alt={`Prévia de ${project.title}`} loading="lazy" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = defaultSiteConfig.profilePhoto; }} />}<span>{project.number}</span></div>
